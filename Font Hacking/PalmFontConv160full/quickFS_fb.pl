...
# 0         1             2           3     4
unless ( @ARGV >= 5 ) { die "quickFS fontstyle stdFileName size1 size2 ..."; }
$i = 2;
while ( $i < @ARGV ) {
    $size1 = $ARGV[ $i ];
    $size2 = $ARGV[ $i + 1 ];
    open O, ">temp.pfj";
    print O "set outFileName      \"Fonts4OS5-$ARGV[0]-$size1-ker.prc\"\n";
